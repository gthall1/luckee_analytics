class GamesController < ApplicationController
    ...
    def index
        ...
        @game_sessions = GameSession.order('game_session_created desc').limit(500)
        @weekly_games = WeeklyDatum.select("date, games_played").where(:date => Time.now-18.weeks..Time.now).order('date asc') #for weekly growth chart
